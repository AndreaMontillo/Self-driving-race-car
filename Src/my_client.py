...
global T
count = 0
pop_size = 20 #CAMBIA ANCHE IN CONFIG
n = 60
n = n+1
...
def cambia_config(filename):
    path_config = "C:\Program Files (x86)\\torcs\config\\raceman\quickrace.xml"
...
def eval_genomes_multi(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        for i in range(0,2):
            ...
            mydoc = minidom.parse("C:\Program Files (x86)\\torcs\config\\raceman\quickrace.xml")
            items = mydoc.getElementsByTagName('attstr')
            print("Inizio gara sul tracciato: ", items[3].attributes['val'].value)
            ...
            for step in range(C.maxSteps, 0, -1):
                C.get_servers_input()
                flag = drive(C, step, genome)
                if flag and step_primo_danno == 0:
                    step_primo_danno = C.maxSteps - step
                C.respond_to_server()
            if not C.stage:
            ...
            C.shutdown()
            if i == 0:
                cambia_config(filename="quickrace_Ruudskogen.xml")


            if i == 0:

                if S['lastLapTime'] == 0:
                    fit1 = (S['distRaced'] / S['curLapTime']) * 3.6
                else:
                    fit1 = ((S['distRaced']) * 3.6) / (S['curLapTime'] + S['lastLapTime'])

                if S['damage'] > 0:
                    if step_primo_danno < 550:
                        fit1 = fit1/10
                    elif step_primo_danno < 840:
                        fit1 = fit1/9
                    elif step_primo_danno < 1000:
                        fit1 = fit1/8
                    elif step_primo_danno < 1150:
                        fit1 = fit1/7
                    elif step_primo_danno < 1600:
                        fit1 = fit1/6
                    elif step_primo_danno < 2700:
                        fit1 = fit1/4
                    elif step_primo_danno < 3200:
                        fit1 = fit1/3
                    elif step_primo_danno < 4000:
                        fit1 = fit1/2
                    else:
                        fit1 = fit1 / 1.5

                # elif S['distRaced'] < 500 and S['damage'] > 2000:
                #     fit1 = fit1/10
                #
                # elif S['distRaced'] < 800 and S['damage'] > 3300:
                #     fit1 = fit1/10
                #
                # elif S['distRaced'] < 1300 and S['damage'] > 4300:
                #     fit1 = fit1/10
                #
                # elif S['distRaced'] < 1550 and S['damage'] > 5000:
                #     fit1 = fit1/10
                #
                # elif S['distRaced'] < 1700 and S['damage'] > 5500:
                #     fit1 = fit1/10
                #
                # elif S['distRaced'] < 2550 and S['damage'] > 6000:
                #     fit1 = fit1/10

            elif i == 1 :
                if S['damage'] <= 0:
                    if S['lastLapTime'] == 0:
                        fit2 = (S['distRaced'] / S['curLapTime']) * 3.6
                    else:
                        fit2 = ((S['distRaced']) * 3.6) / (S['curLapTime'] + S['lastLapTime'])
                else:
                    fit2 = 0

            else:
                print("ERROREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEe")



        print("Fit1: " , fit1)
        print("Fit2: ", fit2)
        if fit1 < 10 or fit2 < 30:
            genome.fitness = (fit1 + fit2)/10
        else:
            genome.fitness = fit1 + fit2
...
def eval_genomes(genomes, config):
    for genome_id, genome in genomes:
        ...
        C.shutdown()
        if S['lastLapTime'] == 0:
            genome.fitness = (S['distRaced']/S['curLapTime'])*3.6
        else:
            genome.fitness = ((S['distRaced']) * 3.6)/(S['curLapTime'] + S['lastLapTime'])

        if genome.fitness == None:
            print("OCCAZZOOOOOOOOOOOOOOOOOOooo")
...
def drive(c,tick,genoma):
    ...
    improved_frontal = max(S['track'][3], S['track'][9], S['track'][15])

    #[-45,-19,-12,-7,-4,-2.5,-1.7,-1,-.5,0,.5,1,1.7,2.5,4,7,12,19,45]

    inputs = [S['angle'], S['speedX'], S['speedY'], S['speedZ'], S['trackPos'], S['track'][0], S['track'][1], improved_frontal, S['track'][17], S['track'][18]]
    ...
    if delta_steer < 0 or delta_steer > 1:
...
if __name__ == "__main__":
    ...
    for i in range (8,10):
        ...
        check = my_checkpointer.MyCheckPointer(1, filename_prefix="popolazioni\seed" + str(i+1) + "\generazione-")
        check.seed = i+1
        population.add_reporter(check)
        population.add_reporter(StatisticsReporter())

        # file_name_prefix = "popolazioni\seed5\generazione-"
        # generazione = 10
        # pop = my_checkpointer.MyCheckPointer.restore_checkpoint(file_name_prefix + str(generazione))
        # population = pop
        # print("dimensione pop: ", len(pop.population))
        # check = my_checkpointer.MyCheckPointer(1, filename_prefix=file_name_prefix)
        # check.seed = i+1
        # population.add_reporter(check)
